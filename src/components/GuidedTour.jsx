...
function GuidedTour({ steps, isOpen, onClose, storageKey }) {
  ...
  const calculateTooltipPosition = useCallback((elementRect, scrollX, scrollY, step, tooltipWidth, tooltipHeight) => {
    ...
    if (position === 'top' || (position === 'auto' && elementTop > viewportHeight / 2)) {
      // Position above
    ...
    // If tooltip would overlap with element, adjust position
    const tooltipBottom = top + tooltipHeight;
    const tooltipRight = left + tooltipWidth;

    // Check for overlap and adjust
    if (position === 'bottom' || position === 'auto') {
      if (top < elementBottom + spacing) {
        // Tooltip overlaps with element, move it further down or to the side
        top = elementBottom + spacing;
        // If still doesn't fit, try positioning to the right
        if (top + tooltipHeight > viewportHeight - viewportPadding) {
          top = elementCenterY - tooltipHeight / 2;
          left = elementRight + spacing;
        }
      ...
      if (tooltipBottom > elementTop - spacing) {
        // Tooltip overlaps with element, move it further up or to the side
        top = elementTop - tooltipHeight - spacing;
        // If still doesn't fit, try positioning to the right
        if (top < viewportPadding) {
          top = elementCenterY - tooltipHeight / 2;
